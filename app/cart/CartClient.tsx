...
export default function CartClient({ initialItems }: { initialItems: Record<string, unknown>[] }) {
    ...
    const handleUpdateQuantity = async (id: string, newQuantity: number) => {
        ...
        try {
            if (newQuantity < 1) {
                ...
                setItems(prev => prev.map(item => String(item.id) === id ? { ...item, quantity: newQuantity } : item))
            }
        } catch (error) {
    ...
    return (
        <div className="space-y-4">
            {items.map((item) => (
                <div key={String(item.id)} className="bg-white/5 border border-white/5 rounded-xl p-4 flex gap-4 hover:border-white/10 transition-colors group">
                    ...
                    <div className="w-24 h-24 bg-white/5 rounded-lg flex-shrink-0 flex items-center justify-center p-2">
                        ...
                        <div className="text-xs text-white/20 text-center">{String(item.product_name)}</div>
                    </div>

                    <div className="flex-grow flex flex-col justify-between">
                        <div className="flex justify-between items-start">
                            <div>
                                <h4 className="font-medium text-lg text-white">{String(item.product_name)}</h4>
                                <p className="text-sm text-white/50">Ceremonial Grade</p>
                            </div>
                            <p className="font-mono text-lg">${(Number(item.price) * Number(item.quantity)).toFixed(2)}</p>
                        </div>

                        <div className="flex items-center justify-between mt-4">
                            <div className="flex items-center gap-3 bg-black/20 rounded-full px-1 py-1 border border-white/5">
                                <button
                                    onClick={() => handleUpdateQuantity(String(item.id), Number(item.quantity) - 1)}
                                    disabled={loadingMap[String(item.id)]}
                                    className="p-1 rounded-full hover:bg-white/10 text-white/50 hover:text-white transition-colors disabled:opacity-50"
                                >
                                    <Minus size={14} />
                                </button>
                                <span className="font-mono text-sm w-4 text-center">{String(item.quantity)}</span>
                                <button
                                    onClick={() => handleUpdateQuantity(String(item.id), Number(item.quantity) + 1)}
                                    disabled={loadingMap[String(item.id)]}
                                    className="p-1 rounded-full hover:bg-white/10 text-white/50 hover:text-white transition-colors disabled:opacity-50"
