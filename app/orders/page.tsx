                                    ...
                                    {JSON.parse(order.items || '[]').map((item: Record<string, unknown>, idx: number) => (
                                        <div key={idx} className="flex justify-between items-center text-sm py-1">
                                            <div className="flex items-center gap-3">
                                                ...
                                                <span className="text-white/80">{String(item.product_name)}</span>
                                                <span className="text-white/30 text-xs">x{String(item.quantity)}</span>
                                            </div>
                                            <span className="font-mono text-white/50">${String(item.price)}</span>
                                        </div>
                                    ))}
                                </div>
                            </div>
