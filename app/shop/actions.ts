...
export async function addToCart(product: { name: string, price: string | number }) {
    ...
    // SECURITY FIX: Prevent price manipulation by looking up the true product price on the server
    ...
    if (existingItem) {
        ...
        // Insert new item using the trusted price
        const { error } = await supabase
            .from('cart_items')
            .insert({
                user_id: user.id,
                product_name: trustedProduct.name,
                price: trustedProduct.price, // Use server-validated price
                quantity: 1
            })
