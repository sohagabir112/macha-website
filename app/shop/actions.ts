...
export async function addToCart(product: { name: string, price: string | number }) {
    ...
    // Security check: validate product and price against server-side catalog
    ...
    const { data: existingItem } = await supabase
        ...
        .select('*')
        .eq('user_id', user.id)
        .eq('product_name', catalogProduct.name)
        .single()
    ...
    if (existingItem) {
        ...
        const { error } = await supabase
            ...
            .insert({
                ...
                price: catalogProduct.price, // Use trusted server-side price
                quantity: 1
            })
