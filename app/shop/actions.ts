...
// Trusted server-side product catalog to prevent client-side price manipulation
const SERVER_PRODUCTS: Record<string, number> = {
...
export async function addToCart(product: { name: string, price: string | number }) {
    ...
    // Validate product against trusted server catalog
    const trustedPrice = SERVER_PRODUCTS[product.name]
    ...
    if (existingItem) {
        ...
        const { error } = await supabase
            ...
            .insert({
                ...
                price: trustedPrice, // Use trusted server price, not client-provided price
                quantity: 1
            })
