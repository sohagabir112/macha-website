...
export async function addToCart(product: { name: string, price: string | number }) {
    ...
    // Client-provided price is untrusted. Look up the valid product and price from the server source of truth.
    ...
    if (existingItem) {
        ...
        const { error } = await supabase
            ...
            .insert({
                ...
                price: serverProduct.price, // Use trusted server-side price
                quantity: 1
            })
