...
export async function addToCart(product: { name: string, price: string | number }) {
    ...
    // Security Fix: Validate product exists and enforce server-side pricing
    // to prevent client-side price manipulation vulnerabilities
    const validProduct = PRODUCTS.find(p => p.name === product.name)
    if (!validProduct) {
        return { error: "Invalid product selected." }
    }
    const securePrice = validProduct.price
