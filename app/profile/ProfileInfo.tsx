...
export default function ProfileInfo({ profile, user, children }: { profile: Record<string, unknown>, user: any, children: React.ReactNode }) {
    ...
    return (
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex flex-col items-center text-center relative group">
            ...
            <p className="text-white/40 text-sm mb-6">@{(profile?.username as string) || (user.email as string)?.split('@')[0]}</p>
