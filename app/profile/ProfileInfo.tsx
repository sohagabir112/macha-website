...
export default function ProfileInfo({ profile, user, children }: { profile: Record<string, unknown> | null, user: SupabaseUser | null, children: React.ReactNode }) {
        ...
        <div className="bg-white/5 backdrop-blur-md border border-white/10 rounded-2xl p-6 flex flex-col items-center text-center relative group">
            ...
            <div className="w-24 h-24 rounded-full bg-gradient-to-br from-matcha/80 to-matcha/20 p-1 mb-4 relative">
                <div className="w-full h-full rounded-full bg-[#0a0a0a] flex items-center justify-center overflow-hidden">
                    {profile?.avatar_url ? (
                        <Image src={profile.avatar_url as string} alt="Profile" width={96} height={96} className="object-cover" />
                    ) : (
                        <User size={40} className="text-matcha" />
                    )}
                </div>
            </div>
            <h2 className="text-xl font-bold text-white mb-1">{(profile?.full_name as string) || 'Matcha Lover'}</h2>
            <p className="text-white/40 text-sm mb-6">@{(profile?.username as string) || user?.email?.split('@')[0]}</p>
